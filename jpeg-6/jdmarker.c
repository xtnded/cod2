...
#define INPUT_BYTE(cinfo, V, action)                                           \
  MAKESTMT(MAKE_BYTE_AVAIL(cinfo, action); V = GETJOCTET(*next_input_byte++);)
  ...
  /* See if a JFIF APP0 marker is present */
    ...
    if (length >= JFIF_LEN) {
      ...
      length -= JFIF_LEN;
        ...
        if (b[0] == 0x4A && b[1] == 0x46 && b[2] == 0x49 && b[3] == 0x46 &&
            b[4] == 0) {
          ...
          TRACEMS3(cinfo, 1, JTRC_JFIF, cinfo->X_density, cinfo->Y_density,
                   cinfo->density_unit);
          if (b[12] | b[13])
  ...
  /* See if an Adobe APP14 marker is present */
    ...
    if (length >= ADOBE_LEN) {
      ...
      length -= ADOBE_LEN;
        ...
        if (b[0] == 0x41 && b[1] == 0x64 && b[2] == 0x6F && b[3] == 0x62 &&
            b[4] == 0x65) {
...
METHODDEF int read_markers(j_decompress_ptr cinfo)
{
    ...
    for (;;) {
        ...
        default: /* must be DHP, EXP, JPGn, or RESn */
          /* For now, we treat the reserved markers as fatal errors since they
           * are likely to be used to signal incompatible JPEG Part 3
           * extensions. Once the JPEG 3 version-number marker is well defined,
           * this code ought to change!
           */
...
GLOBAL void jinit_marker_reader(j_decompress_ptr cinfo)
{
  ...
  cinfo->marker = (struct jpeg_marker_reader *)(*cinfo->mem->alloc_small)(
      (j_common_ptr)cinfo, JPOOL_PERMANENT, SIZEOF(struct jpeg_marker_reader));
