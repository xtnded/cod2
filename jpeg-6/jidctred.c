...
#define DEQUANTIZE(coef, quantval) (((ISLOW_MULT_TYPE)(coef)) * (quantval))
  ...
  for (ctr = 0; ctr < 4; ctr++) {
    ...
    if ((wsptr[1] | wsptr[2] | wsptr[3] | wsptr[5] | wsptr[6] | wsptr[7]) ==
        0) {
      /* AC terms all zero */
      JSAMPLE dcval =
...
GLOBAL void jpeg_idct_2x2(j_decompress_ptr cinfo, jpeg_component_info *compptr,
  ...
  for (ctr = DCTSIZE; ctr > 0; inptr++, quantptr++, wsptr++, ctr--) {
    ...
    if (ctr == DCTSIZE - 2 || ctr == DCTSIZE - 4 || ctr == DCTSIZE - 6)
      continue;
    if ((inptr[DCTSIZE * 1] | inptr[DCTSIZE * 3] | inptr[DCTSIZE * 5] |
         inptr[DCTSIZE * 7]) == 0) {
...
GLOBAL void jpeg_idct_1x1(j_decompress_ptr cinfo, jpeg_component_info *compptr,
                          JCOEFPTR coef_block, JSAMPARRAY output_buf,
                          JDIMENSION output_col) {
  int dcval;
  ISLOW_MULT_TYPE *quantptr;
