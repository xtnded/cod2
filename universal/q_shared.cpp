...
float __cdecl IntAsFloat(int) { UNIMPLEMENTED(); }
...
bool __cdecl I_isdigit(int c) { return (unsigned int)(a1 - 48) <= 9; }

bool __cdecl I_isalpha(int) { UNIMPLEMENTED(); }

bool __cdecl I_isupper(int c) { return (unsigned int)(a1 - 65) <= 0x19; }

bool __cdecl I_islower(int c) { return (unsigned int)(a1 - 97) <= 0x19; }

bool __cdecl I_isprint(int) { UNIMPLEMENTED(); }
...
unsigned __int64 __cdecl Long64NoSwap(unsigned __int64 ll) { return a1; }
...
int __cdecl LongNoSwap(int l) { return a1; }

int __cdecl LongSwap(int l) {
  return HIBYTE(a1) + ((a1 >> 8) & 0xFF00) + (BYTE1(a1) << 16) + (a1 << 24);
}

short __cdecl ShortNoSwap(short l) { return a1; }

short __cdecl ShortSwap(short l) { return (__int16)__ROL2__(a1, 8); }

int __cdecl LittleFloatWrite_Util(float) { UNIMPLEMENTED(); }

float __cdecl LittleFloatRead_Util(int) { UNIMPLEMENTED(); }

unsigned __int64 __cdecl LittleLong64(unsigned __int64) { UNIMPLEMENTED(); }
...
void __cdecl Info_SetValueForKey(char *s, char const *key, char const *value) {
  ...
  if (strlen(__s1) > 0x3FF) {
    ...
    if (strchr(__s, 92)) {
      ...
      Com_Printf("\x15Can't use keys with a semicolon\nkey: '%s'\nvalue: '%s'",
                 __s, a3);
      ...
      if (v6[0]) {
        if (Com_sprintf(__s2, 0x400u, "\\%s\\%s", __s, v6) <= 0) {
          Com_Printf("\x15Info buffer length exceeded, not including key/value "
                     "pair in response\n");
        } else if (strlen(__s1) + 1 + strlen(__s2) + 1 - 2 <= 0x400) {
          strcat(__s1, __s2);
        } else {
          Com_Printf("\x15Info string length exceeded\nkey: '%s'\nvalue: "
                     "'%s'\nInfo string:\n%s\n",
                     __s, a3, __s1);
        }
      }
...
bool __cdecl I_isalnum(int) { UNIMPLEMENTED(); }

int __cdecl FloatWriteNoSwap(float f) { return LODWORD(a1); }

float __cdecl FloatReadNoSwap(int n) { return *(float *)&a1; }

void __cdecl Com_StripFilename(char const *, char *) { UNIMPLEMENTED(); }
...
void __cdecl Info_RemoveKey_Big(char *s, char const *key) { UNIMPLEMENTED(); }
