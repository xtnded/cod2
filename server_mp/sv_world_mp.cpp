    ...
    for (i = 0; i != v9; ++i) {
      if (v10[i] != a2) {
        ...
        if (*(_BYTE *)(v6 + 241))
          ...
          v5 = CM_TempBoxModel((float *)(v6 + 260), (const float *)(v6 + 272),
                               *(_DWORD *)(v6 + 284));
        v3 |= CM_TransformedPointContents(a1, v5, (const float *)(v7 + 312),
                                          (float *)(v7 + 324));
      }
    }
  }
...
int __cdecl SV_ClipSightToEntity(struct sightclip_t const *clip,
  ...
  if (!*(_BYTE *)(v3 + 241))
    v9 = vec3_origin;
  if (CM_TransformedBoxSightTrace(0, a1->start, a1->end, a1->mins, a1->maxs, v8,
                                  contentmask, (const float *)(v3 + 312), v9)) {
    return -1;
  } else {
    return 0;
  }
...
int __cdecl SV_PointSightTraceToEntity(struct sightpointtrace_t const *clip,
  ...
  if (passEntityNum_high != 1023 &&
    ...
    return 0;
  if (!a1->locational || (ServerDObj = Com_GetServerDObj(v3->s.number)) == 0 ||
      (v3->r.svFlags & 4) == 0) {
    if (v3->r.bmodel)
...
void __cdecl SV_PointTraceToEntity(struct pointtrace_t const *clip,
  ...
  LABEL_7:
    if ((v4 & a2->linkcontents) != 0 &&
        !CM_TraceBox(a1, v3->r.absmin, v3->r.absmax, LODWORD(a3->fraction))) {
      ...
      if (!v3->r.bmodel)
        currentAngles = vec3_origin;
      fraction = a3->fraction;
      CM_TransformedBoxTrace(a3, a1->start, a1->end, vec3_origin, vec3_origin,
                             v7, (float *)LODWORD(a1[1].start[2]),
                             v3->r.currentOrigin, currentAngles);
      if (a3->fraction < fraction) {
        a3->surfaceFlags = 0;
        *(_WORD *)a3->partName = 0;
...
void __cdecl SV_SightTrace(int *hitNum, float const *const start,
  ...
  *a1 = result;
  if (!result) {
    ...
    v10 = a3 + 2;
    v11 = *a4;
    if ((float)((float)((float)((float)((float)(*a4 - *a3) + a4[1]) - a3[1]) +
                        a4[2]) -
                a3[2]) == 0.0) {
...
void __cdecl SV_Trace(struct trace_t *results, float const *const start,
  ...
  if (a1->fraction != 0.0) {
    if (!a10 || (result = (unsigned __int16)CM_PointTraceStaticModels(
      ...
      v23 = &a3->normal[1];
      v19 = *a4;
      if ((float)((float)((float)((float)((float)(*a4 - a3->fraction) + a4[1]) -
                                  a3->normal[0]) +
                          a4[2]) -
                  a3->normal[1]) == 0.0) {
        ...
        *(_DWORD *)&v24.extents[16] = a9;
        if (a6 == 1023 ||
            (*(_DWORD *)&v24.extents[4] = *(_DWORD *)(SV_GentityNum(a6) + 336),
             *(_DWORD *)&v24.extents[4] == 1023)) {
          *(_DWORD *)&v24.extents[4] = -1;
        }
        return (unsigned __int16)CM_PointTraceToEntities((pointtrace_t *)&v24,
                                                         a1);
      } else {
...
void __cdecl SnapAngles(float *const) { UNIMPLEMENTED(); }
