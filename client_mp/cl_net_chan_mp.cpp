void __cdecl CL_Netchan_UpdateProfileStats(void) { UNIMPLEMENTED(); }
...
void __cdecl CL_Netchan_PrintProfileStats(int bPrintToConsole) {
  ...
  if (a2) {
    ...
    LABEL_9:
      v6 = v4[181];
      v7 = v4[369];
      Com_sprintf(__str, 0x400u, "    OOB Sent: %5i %5i %5i    -", v6, v4[186],
                  v4[187]);
      if (a2) {
        Com_Printf("%s\n", __str);
      } else {
...
void __cdecl CL_ProfDraw(int, char const *) { UNIMPLEMENTED(); }

CL_Netchan_Decode(unsigned char *data, int size) { UNIMPLEMENTED(); }
