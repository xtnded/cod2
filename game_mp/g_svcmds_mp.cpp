void __cdecl Svcmd_EntityList_f() { UNIMPLEMENTED(); }
      ...
      if (numIPFilters > 0) {
        ...
        do {
          if (v9[1] != -1) {
            v14[0] = v9[1];
            v10 = strlen(__src) + 1;
            Com_sprintf(&__src[v10 - 1], 1024 - (v10 - 1), "%i.%i.%i.%i ",
                        LOBYTE(v14[0]), BYTE1(v14[0]), BYTE2(v14[0]),
                        HIBYTE(v14[0]));
          }
          ++v11;
          v9 += 2;
...
void __cdecl Svcmd_AddIP_f(void) { UNIMPLEMENTED(); }
...
int __cdecl StringToFilter(char *, struct ipFilter_s *) { UNIMPLEMENTED(); }

void __cdecl UpdateIPBans(void) { UNIMPLEMENTED(); }
