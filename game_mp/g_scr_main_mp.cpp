              ...
              v5 = FS_FOpenFileByMode(v4, &v16, FS_READ);
                if ((unsigned int)(v5 - 1) <= 0x3FE) {
                  ...
                  *((_DWORD *)v9 + 35) = v8 && !I_stricmp(v8, "team");
                  ...
                  if (v5 <= 0)
                    Com_Printf(
                        "WARNING: Could not load GameType description file %s "
                        "for gametype %s\n",
                        v6, v1);
                  else
                    Com_Printf("WARNING: GameType description file %s is too "
                               "big to load.\n",
                               v6);
  ...
  char *v3;           // eax
    ...
    if (Scr_GetNumParam()) {
      ...
      SV_GameSendServerCommand(
          -1963413621 * (((char *)Entity - (char *)&g_entities) >> 4),
    ...
    for (i = 31; i != 46; ++i) {
      SV_GetConfigstring(a1, a2, a3, a4, a5, a6, a7, i, v12, 1024);
        if (!I_stricmp(v12, __s1)) {
          Com_DPrintf("Script tried to precache the player head icon '%s' more "
                      "than once\n",
                      __s1);
          return;
        }
  ...
  v8 = 0;
    while (1) {
      ...
      if (!v12[0])
        ...
        if (++v8 == 15) {
          v10 =
              va("Too many player head icons precached. Max allowed is %i", 15);
          Scr_Error(v10);
          v9 = 46;
          break;
        }
  ...
  __s1 = (char *)Scr_GetString(a2, 0);
    for (i = 23; i != 31; ++i) {
      SV_GetConfigstring(a1, a2, a3, a4, a5, a6, a7, i, v12, 1024);
        if (!I_stricmp(v12, __s1)) {
          Com_DPrintf(
              "Script tried to precache the player status icon '%s' more "
              "than once\n",
              __s1);
          return;
        }
  ...
  v8 = 0;
    while (1) {
      ...
      if (!v12[0])
        break;
        if (++v8 == 8) {
          v10 = va("Too many player status icons precached. Max allowed is %i",
                   8);
          Scr_Error(v10);
          v9 = 31;
          break;
...
{
  return Scr_AddInt(dword_1934678);
}
    ...
    if (!*(_BYTE *)(g_NoScriptSpam + 8)) {
      NumParam = Scr_GetNumParam();
        if (NumParam > 0) {
          v8 = 0;
            do {
              DebugString = (const char *)Scr_GetDebugString(a1, a2, a3, a4, a5,
                                                             a6, a7, v8);
