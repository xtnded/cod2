...
float __cdecl CanDamage(struct gentity_s *targ, float const *const origin)
{
  ...
  float v35;    // [esp+9Ch] [ebp-1Ch]
    ...
    if (a1->client) {
      ...
      v9 = &v16;
        for (i = a1;; i = a1) {
          v7 -= (G_LocationalTracePassed(a2, v9, i->s.number,
                                         (int)&unk_802891) == 0) -
                1;
          ++v8;
          v9 += 3;
                      ...
                      enum hitLocation_t hitLoc, int timeOffset)
{
  ...
  client = (_DWORD *)a8->client;
    if (client) {
        if (a8->takedamage && !client[2539] && !client[2540] &&
            client[2481] == 2) {
            if (a9) {
              weapon = a9->s.weapon;
            }
        ...
        if ((flags & 1) == 0) {
          ...
          v24 = a8->health;
            if (v24 <= 0) {
              ...
              v27 = (void(__cdecl *)(
                  gentity_s *, gentity_s *, gentity_s *, int, unsigned int,
                  int32_t, float *, hitLocation_t,
