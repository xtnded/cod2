    ...
    if (a8.classnum) {
      ...
      v9 = *(_WORD *)v8->classname;
        if (v9 != word_195B0C2 && v9 != word_195B0C4) {
            ...
            LABEL_10:
              Com_DPrintf(
                  "cannot use the solid/notsolid commands on a script_origin "
                  "entity( number %i )\n",
                  v8->s.number);
              return;
            }
...
void __cdecl ScriptEntCmd_RotateVelocity(struct scr_entref_t entref)
{
  ...
  apos = v1->s.apos;
    ...
    if (v4 == 0.0 && v14 == 0.0) {
      ...
      *p_accumulate = v14;
      v1->unnamed_field.mover.aSpeed =
          fsqrt((float)((float)(v19 * v19) + (float)(v20 * v20)) +
                (float)(v21 * v21));
        if (v4 == 0.0) {
          *apos1 = *currentAngles;
          ...
          *(_DWORD *)apos = 7;
          BG_EvaluateTrajectory((const trajectory_t *)v1->s.apos,
                                *(_DWORD *)&v1->s.apos[8] + dword_193466C,
                                apos1);
        }
      v8 = *p_aMidTime;
    ...
    if (a1->s.pos.trType) {
        if ((signed int)(a1->s.pos.trDuration + a1->s.pos.trTime) <=
            ...
            (speed = a1->unnamed_field.mover.speed,
             ...
             BG_EvaluateTrajectory(&a1->s.pos, dword_193466C,
                                   a1->r.currentOrigin),
             SV_LinkEntity(a1), v5)) {
      ...
      result = (char **)&result[13][(_DWORD)result[14]];
        if ((int)result <= dword_193466C) {
          ...
          result = (char **)SV_LinkEntity(a1);
            if (v3) {
              a1->r.currentAngles[0] =
                  AngleNormalize180(a1->r.currentAngles[0]);
              a1->r.currentAngles[1] =
                  AngleNormalize360(a1->r.currentAngles[1]);
              a1->r.currentAngles[2] =
                  AngleNormalize180(a1->r.currentAngles[2]);
