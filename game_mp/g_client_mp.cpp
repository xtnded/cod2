  ...
  *(_DWORD *)(result + 10248) = dword_193466C;
  ...
  v4 = &g_entities;
    do {
        ...
        if (*(_BYTE *)(voice_global + 8) ||
            (result = OnSameTeam(a1, v4)) != 0) {
          client = a1->client;
        LABEL_3:
          ...
          v7 = *(_DWORD *)(client + 9896);
            if (v6 == v7 ||
                ...
                if (a1 != v4 || (result = voice_localEcho,
                                 *(_BYTE *)(voice_localEcho + 8))) {
...
void __cdecl SetClientViewAngle(struct gentity_s *ent, float const *const angle)
{
  float v2;       // ecx
  int32_t client; // edx
  int v4;         // ecx
...
int __cdecl G_GetNonPVSFriendlyInfo(struct gentity_s *pSelf,
                                    float *const vPosition, int iLastUpdateEnt)
{
  UNIMPLEMENTED();
}

void __cdecl ClientBegin(int)
{
  UNIMPLEMENTED();
}
  ...
  client = a1->client;
    if ((*(_DWORD *)(client + 160) & 0x300) != 0) {
      ...
      *a2 = *(float *)(client + 20);
      ...
      VerticalBobFactor =
          BG_GetVerticalBobFactor((const playerState_s *)client, BobCycle,
...
void __cdecl ClientCleanName(char const *, char *, int)
{
  UNIMPLEMENTED();
}
